...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw >=200){
    analogWrite(PIN_LED, (dist_raw - 200)*255/100);
  }
  else{
    analogWrite(PIN_LED, (200-dist_raw)*255/100);
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min) reading = 100;
  if(reading > dist_max) reading = 300; // return 0 when out of range.
  return reading;
